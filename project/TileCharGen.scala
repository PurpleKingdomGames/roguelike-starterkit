...
object TileCharGen {
  ...
  def genFont(charWidth: Int, charHeight: Int, chars: List[CharDetail]): String = {
    ...
    val charString = chars
      .map(cd => "            " + cd.toFontChar(charWidth, charHeight) + ",")
      .mkString("\n")
  ...
  def templateTile(fullyQualifiedPath: String, chars: List[CharDetail]): String = {
    val charString =
      chars
        .map { cd =>
          ...
          s"""  ${c
              .map { cc =>
                if (cd.name == "NULL") "// Null char"
                else s"val `$cc`: Tile = ${cd.index.toString}"
              }
    ...
    s"""package $fullyQualifiedPath
    |
    |// GENERATED by Roguelike-Starterkit: TileCharGen.scala - DO NOT EDIT
    |/** A `Tile` is an index (between 0-255 inclusive) onto a 16x16 tile sheet.*/
    |opaque type Tile = Int
    |
    |object Tile {
    |  /** Unsafe `Tile` constructor */
    |  inline def apply(char: Int): Tile = char
    |
    |  /** Safe construction of a `Tile` instance, sanitised to the right range. */

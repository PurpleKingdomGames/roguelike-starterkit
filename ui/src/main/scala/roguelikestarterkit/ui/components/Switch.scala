...
/** The Switch `Component` allows you to create a two state (on / off) button for your UI. These can
  * be used for simple checkboxes, toggles, and switches, but also with more coordination, compound
  * components like radio button groups.
  */
final case class Switch[ReferenceData](
  ...
  /** Decide the state of the switch based on the current state and the reference data. Returns an
    * optional value, if `None` is returned the switch will not change state.
    */
  def withAutoToggle(
